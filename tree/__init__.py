...
def _get_attrs_items(obj):
    ...
    """
    return [
        (attr.name, getattr(obj, attr.name)) for attr in obj.__class__.__attrs_attrs__
    ]
...
def _yield_flat_up_to(shallow_tree, input_tree, path=()):
    ...
    Yields:
      ...
      input_tree.
    """
    if isinstance(shallow_tree, _TEXT_OR_BYTES) or not (
        isinstance(shallow_tree, (collections_abc.Mapping, collections_abc.Sequence))
        or _is_namedtuple(shallow_tree)
        or _is_attrs(shallow_tree)
    ):
        ...
        def traverse_subtrees():
            if is_nested(structure):
                return _sequence_like(
                    structure, map(subtree_fn, _yield_sorted_items(structure))
                )
            else:
                return structure
